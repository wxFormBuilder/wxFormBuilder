    ...
    switch (type) {
        ...
        case PT_WXCOLOUR: {
            if (!value.empty()) {
                if (value.find_first_of(wxT("wx")) == 0) {
                    ...
                    result =
                      wxString::Format(wxT("new wxColour( %i, %i, %i )"), colour.Red(), colour.Green(), colour.Blue());
                }
            } else {
        ...
        case PT_BITMAP: {
            ...
            if (source == wxT("Load From File")) {
                ...
                result << wxT("new wxBitmap( \"") << PHPCodeGenerator::ConvertPHPString(file)
                ...
                if (wxDefaultSize == icoSize) {
                    ...
                    result.Printf(
                      wxT("new wxIcon( \"%s\", wxBITMAP_TYPE_ICO_RESOURCE, %i, %i )"), path, icoSize.GetWidth(),
                      icoSize.GetHeight());
                }
...
bool PHPCodeGenerator::GenEventEntry(
        ...
        if (!_template.empty()) {
            if (disconnect) {
                if (m_disconnecMode == wxT("handler_name"))
                    _template.Replace(wxT("#handler"), wxT("handler = array(@$this, \"") + handlerName + wxT("\")"));
                else if (m_disconnecMode == wxT("source_name"))
                    _template.Replace(
                      wxT("#handler"),
                      wxT("null"));  // wxT("$this->") + obj->GetProperty(wxT("name"))->GetValueAsString() );
            } else
...
void PHPCodeGenerator::GenClassDeclaration(
  ...
  const wxString& eventHandlerPostfix, ArrayItems& arrays)
{
    ...
    m_source->WriteLn(
      wxT("class ") + classDecoration + class_name + wxT(" extends ") + GetCode(class_obj, wxT("base")).Trim() +
      wxT(" {"));
    m_source->Indent();
...
void PHPCodeGenerator::GenSubclassSets(
...
void PHPCodeGenerator::GenObjectIncludes(
...
void PHPCodeGenerator::GenBaseIncludes(
...
void PHPCodeGenerator::GenConstruction(PObjectBase obj, bool is_widget, ArrayItems& arrays)
{
    ...
    if (ObjectDatabase::HasCppProperties(type)) {
        ...
        if (!isWidget)  // sizers
        {
          ...
          type == wxT("ribbonbar") || type == wxT("listbook") || type == wxT("simplebook") || type == wxT("notebook") ||
