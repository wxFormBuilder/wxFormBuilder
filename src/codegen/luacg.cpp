...
LuaTemplateParser::LuaTemplateParser(const LuaTemplateParser& that, const wxString& _template) :
    TemplateParser(that, _template),
    m_i18n(that.m_i18n),
    m_useRelativePath(that.m_useRelativePath),
    m_basePath(that.m_basePath),
    m_rootWxParent(that.m_rootWxParent),
    ...
    switch (type) {
        ...
        case PT_EDIT_OPTION: {
            ...
            if (auto pred = m_predModulePrefix.find(value); pred != m_predModulePrefix.end() && !pred->second.empty()) {
                ...
                if (
                  ...
                  wxNOT_FOUND != result.Find(wxT("wx"))) {
        ...
        case PT_WXCOLOUR: {
            if (!value.empty()) {
                if (value.find_first_of(wxT("wx")) == 0) {
                    ...
                    result =
                      wxString::Format(wxT("wx.wxColour( %i, %i, %i )"), colour.Red(), colour.Green(), colour.Blue());
                }
            } else {
        ...
        case PT_BITMAP: {
            ...
            if (source == _("Load From File") || source == _("Load From Embedded File")) {
                ...
                result << wxT("wx.wxBitmap( \"") << LuaCodeGenerator::ConvertLuaString(file)
                ...
                // wxStaticBitmap(wxWindow* parent, wxWindowID id, const wxBitmap& label = wxNullBitmap, const wxPoint&
                ...
                {
                        result.Printf( wxT("wx.Icon( u\"%s\", wx.wxBITMAP_TYPE_ICO_RESOURCE, %i, %i )"), path,
                ...
                wxLogWarning(
                  wxT("Lua code generation does not support using SVG resources for bitmap properties:\n%s"), path);
                result = wxT("wx.wxNullBitmap");
            }
            break;
        }
...
void LuaCodeGenerator::GenerateInheritedClass(
  PObjectBase userClasses, PObjectBase form, const wxString& genFileFullPath)
{
    if (!userClasses) {
        ...
        for (size_t i = 0; i < events.size(); i++) {
            ...
            if (propName) {
                ...
                if (strPrevClassName != strClassName) {
                    ...
                    bAddCaption = true;
                    eventsGroupID =
                      wxString::Format(wxT("-- %s (%s) event handlers: "), strClassName, obj->GetClassName());
                }
            }
...
wxString LuaCodeGenerator::GenEventEntryForInheritedClass(
  ...
  wxString& strClassName)
{
    ...
    if (code_info) {
        ...
        LuaTemplateParser parser(obj, _template, m_i18n, m_useRelativePath, m_basePath, strClassName, m_strUserIDsVec);
        ...
        for (unsigned int i = 0; i < obj_info->GetBaseClassCount(false); i++) {
            ...
            if (
              (code = GenEventEntryForInheritedClass(obj, base_info, templateName, handlerName, strClassName))
                .length() > 0) {
                return code;
            }
        }
...
bool LuaCodeGenerator::GenerateCode(PObjectBase project)
{
    ...
    for (unsigned int i = 0; i < dProjChildCount; i++) {
        ...
        GenClassDeclaration(child, false, wxEmptyString, events, m_strEventHandlerPostfix, arrays);
    }
...
void LuaCodeGenerator::GenEvents(
  PObjectBase class_obj, const EventVector& events, wxString& strClassName, bool disconnect)
{
    if (events.empty()) {
        return;
    }
...
bool LuaCodeGenerator::GenEventEntry(
  PObjectBase obj, PObjectInfo obj_info, const wxString& templateName, const wxString& handlerName,
    ...
    if (code_info) {
        ...
        if (!_template.empty()) {
            ...
            LuaTemplateParser parser(
              obj, _template, m_i18n, m_useRelativePath, m_basePath, strClassName, m_strUserIDsVec);
...
void LuaCodeGenerator::GenVirtualEventHandlers(
...
void LuaCodeGenerator::GetGenEventHandlers(PObjectBase obj, wxString& strClassName)
{
    GenDefinedEventHandlers(obj->GetObjectInfo(), obj, strClassName);

    for (unsigned int i = 0; i < obj->GetChildCount(); i++) {
        PObjectBase child = obj->GetChild(i);
        GetGenEventHandlers(child, strClassName);
    }
}

void LuaCodeGenerator::GenDefinedEventHandlers(PObjectInfo info, PObjectBase obj, wxString& strClassName)
{
    ...
    for (unsigned int i = 0; i < info->GetBaseClassCount(false); i++) {
        ...
        GenDefinedEventHandlers(base_info, obj, strClassName);
    }
}


wxString LuaCodeGenerator::GetCode(PObjectBase obj, wxString name, bool silent, wxString strSelf)
{
    wxString _template;
    ...
    LuaTemplateParser parser(obj, _template, m_i18n, m_useRelativePath, m_basePath, strSelf, m_strUserIDsVec);
    wxString code = parser.ParseTemplate();
...
wxString LuaCodeGenerator::GetConstruction(PObjectBase obj, bool silent, wxString strSelf, ArrayItems& arrays)
{
    // Get the name
...
void LuaCodeGenerator::GenClassDeclaration(
  ...
  const wxString& /*eventHandlerPostfix*/, ArrayItems& arrays)
{
    ...
    PProperty propName = class_obj->GetProperty(wxT("name"));
    if (!propName) {
        wxLogError(wxT("Missing \"name\" property on \"%s\" class. Review your XML object description"), strClassName);
        return;
    }
    ...
    GetGenEventHandlers(class_obj, strName);
    GenConstructor(class_obj, events, strName, arrays);
}

void LuaCodeGenerator::GenSubclassSets(
...
void LuaCodeGenerator::GenObjectIncludes(
  PObjectBase project, std::vector<wxString>* includes, std::set<wxString>* templates)
{
    ...
    if (code_info) {
        LuaTemplateParser parser(
          project, code_info->GetTemplate(wxT("include")), m_i18n, m_useRelativePath, m_basePath, wxEmptyString, {});
...
void LuaCodeGenerator::GenBaseIncludes(
  PObjectInfo info, PObjectBase obj, std::vector<wxString>* includes, std::set<wxString>* templates)
{
    ...
    if (code_info) {
        LuaTemplateParser parser(
          obj, code_info->GetTemplate(wxT("include")), m_i18n, m_useRelativePath, m_basePath, wxEmptyString, {});
        wxString include = parser.ParseTemplate();
        if (!include.empty()) {
            if (templates->insert(include).second) {
                AddUniqueIncludes(include, includes);
...
void LuaCodeGenerator::GenConstructor(
  PObjectBase class_obj, const EventVector& events, wxString& strClassName, ArrayItems& arrays)
{
    PProperty propName = class_obj->GetProperty(wxT("name"));
    if (!propName) {
        wxLogError(
    ...
    m_source->WriteLn(strName + wxT(" = ") + GetCode(class_obj, wxT("cons_call"), false, strClassName));

    m_source->Indent();
    wxString settings = GetCode(class_obj, wxT("settings"), false, strClassName);
    if (!settings.IsEmpty()) {
    ...
    if (class_obj->GetObjectTypeName() == wxT("wizard") && class_obj->GetChildCount() > 0) {
        ...
        m_source->Indent();
        m_source->WriteLn(
          wxT("local previous_page = ") + m_strUITable + wxT(".") + strClassName + wxT(".m_pages[ #") + m_strUITable +
          wxT(".") + strClassName + wxT(".m_pages ]"));
    ...
    wxString afterAddChild = GetCode(class_obj, wxT("after_addchild"), false, strClassName);
    ...
    auto afterConnectEvents = GetCode(class_obj, wxT("after_connectevents"), false, strClassName);
...
void LuaCodeGenerator::GenDestructor(PObjectBase class_obj, const EventVector& events)
{
    ...
    // FIXME: This is wrong, but since this method is unused, it doesn't matter now
    wxString strClassName;
    ...
    // destruct objects
    GenDestruction(class_obj, strClassName);
...
void LuaCodeGenerator::GenConstruction(PObjectBase obj, bool is_widget, wxString& strClassName, ArrayItems& arrays)
{
    ...
    if (ObjectDatabase::HasCppProperties(type)) {
        ...
        if (
          m_strUnsupportedClasses.end() !=
          (itr = std::find(m_strUnsupportedClasses.begin(), m_strUnsupportedClasses.end(), strClass))) {

            m_source->WriteLn(
              wxT("--Instance ") + strName + wxT(" of Control ") + *itr +
        ...
        for (unsigned int i = 0; i < obj->GetChildCount(); i++) {
            ...
            if (type == wxT("toolbar")) {
                GenAddToolbar(child->GetObjectInfo(), child, strClassName);
            }
        }

        if (!isWidget)  // sizers
        {
            wxString afterAddChild = GetCode(obj, wxT("after_addchild"), false, strClassName);
            ...
            switch (obj->GetChildCount()) {
                ...
                case 2: {
                    ...
                    _template = _template + wxT("#utbl#nop ") + sub1->GetProperty(wxT("name"))->GetValue() +
                                wxT(", #utbl#nop ") + sub2->GetProperty(wxT("name"))->GetValue() + wxT(", $sashpos )");
          ...
          type == wxT("menubar") || type == wxT("menu") || type == wxT("submenu") || type == wxT("toolbar") ||
          type == wxT("tool") || type == wxT("listbook") || type == wxT("simplebook") || type == wxT("notebook") ||
          type == wxT("auinotebook") || type == wxT("treelistctrl") || type == wxT("flatnotebook") ||
          type == wxT("wizard")) {
            wxString afterAddChild = GetCode(obj, wxT("after_addchild"), false, strClassName);
            if (!afterAddChild.empty()) {
                m_source->WriteLn(afterAddChild);
        ...
        m_source->WriteLn(GetCode(obj, temp_name, false, strClassName));
      ...
      type == wxT("simplebookpage") || type == wxT("choicebookpage") || type == wxT("auinotebookpage")) {
        GenConstruction(obj->GetChild(0), false, strClassName, arrays);
        m_source->WriteLn(GetCode(obj, wxT("page_add"), false, strClassName));
        GenSettings(obj->GetObjectInfo(), obj, strClassName);
    } else if (type == wxT("treelistctrlcolumn")) {
        m_source->WriteLn(GetCode(obj, wxT("column_add"), false, strClassName));
        GenSettings(obj->GetObjectInfo(), obj, strClassName);
...
void LuaCodeGenerator::GenDestruction(PObjectBase obj, wxString& strClassName)
{
    wxString _template;
    PCodeInfo code_info = obj->GetObjectInfo()->GetCodeInfo(wxT("Lua"));
    ...
    for (unsigned int i = 0; i < obj->GetChildCount(); i++) {
        PObjectBase child = obj->GetChild(i);
        GenDestruction(child, strClassName);
    }
}
...
void LuaCodeGenerator::GenAddToolbar(PObjectInfo info, PObjectBase obj, wxString& strClassName)
{
    wxArrayString arrCode;

    GetAddToolbarCode(info, obj, strClassName, arrCode);

    for (size_t i = 0; i < arrCode.GetCount(); i++)
        m_source->WriteLn(arrCode[i]);
}

void LuaCodeGenerator::GetAddToolbarCode(
  PObjectInfo info, PObjectBase obj, wxString& strClassName, wxArrayString& codelines)
{
    wxString _template;
    PCodeInfo code_info = info->GetCodeInfo(wxT("Lua"));
    ...
    for (unsigned int i = 0; i < info->GetBaseClassCount(false); i++) {
        PObjectInfo base_info = info->GetBaseClass(i, false);
        GetAddToolbarCode(base_info, obj, strClassName, codelines);
    }
}
