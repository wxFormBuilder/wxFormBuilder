...
CodeWriter::CodeWriter() : m_indent(0), m_isLineWriting(false), m_indent_with_spaces(false)
{
}
...
void CodeWriter::ProcessLine(wxString line, bool rawIndents)
{
    ...
    line.Trim();
    // Remove and count indentations defined in code templates by #indent and #unindent macros to use own indentation
    // mode
...
void CodeWriter::WriteLn(const wxString& code, bool rawIndents)
{
    if (!IsSingleLine(code)) {
        ...
        while (tkz.HasMoreTokens()) { ProcessLine(tkz.GetNextToken(), rawIndents); }
    } else {
...
void CodeWriter::Write(const wxString& code, bool rawIndents)
{
    ...
    if (!m_isLineWriting) {
        if (!rawIndents) {
            for (int i = 0; i < m_indent; ++i) { DoWrite(m_indent_with_spaces ? wxT("    ") : wxT("\t")); }
        }
...
TCCodeWriter::TCCodeWriter() : m_tc(0)
{
}
