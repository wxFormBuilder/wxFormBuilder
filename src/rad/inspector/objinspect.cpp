...
ObjectInspector::ObjectInspector(wxWindow* parent, int id, int style) : wxPanel(parent, id), m_style(style)
{
...
wxPGProperty* ObjectInspector::GetProperty(PProperty prop)
{
    ...
    if (type == PT_MACRO) {
        ...
        for (it = options.begin(); it != options.end(); ++it) { constants.Add(it->first, 1 << index++); }
        ...
        result = new wxStringProperty(
          name, wxPG_LABEL,
          IntList(prop->GetValueAsString(), type == PT_UINTLIST, (PT_INTPAIRLIST == type || PT_UINTPAIRLIST == type))
            .ToString(true));
...
void ObjectInspector::AddItems(
  const wxString& name, PObjectBase obj, PObjectInfo obj_info, PPropertyCategory category, PropertyMap& properties)
{
...
void ObjectInspector::AddItems(
  const wxString& name, PObjectBase obj, PObjectInfo obj_info, PPropertyCategory category, EventMap& events)
{
    ...
    if (it != m_propMap.end()) {
        ...
        switch (prop->GetType()) {
            ...
            case PT_UINTPAIRLIST: {
                IntList il(
                  event.GetPropertyValue(), PT_UINTLIST == prop->GetType(),
                  (PT_INTPAIRLIST == prop->GetType() || PT_UINTPAIRLIST == prop->GetType()));
