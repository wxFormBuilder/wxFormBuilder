...
ObjectTree::ObjectTree(wxWindow* parent, int id) : wxPanel(parent, id)
{
...
void ObjectTree::RestoreItemStatus(PObjectBase obj)
{
    ...
    for (i = 0; i < count; i++) RestoreItemStatus(obj->GetChild(i));
}
...
void ObjectTree::ClearMap(PObjectBase obj)
{
    ...
    for (unsigned int i = 0; i < obj->GetChildCount(); i++) { ClearMap(obj->GetChild(i)); }
}
                ...
                if (me.ShowModal() == wxID_OK) {
                    if (obj->GetClassName() == wxT("wxMenuBar")) {
                        ...
                        AppData()->InsertObject(
                          me.GetMenubar(AppData()->GetObjectDatabase()), AppData()->GetSelectedForm());
                }
            }
        } break;
        default:
...
void ItemPopupMenu::OnUpdateEvent(wxUpdateUIEvent& e)
{
    switch (e.GetId()) {
        case MENU_EDIT_MENUS:
            e.Enable(
              m_object && (m_object->GetClassName() == wxT("wxMenuBar") || m_object->GetClassName() == wxT("Frame")));
            break;
        case MENU_CUT:
        case MENU_COPY:
        case MENU_DELETE:
