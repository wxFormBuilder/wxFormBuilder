...
public:
    XrcPreviewPopupMenu(wxWindow* window) : m_window(window) { Append(MENU_DELETE, wxT("Close Preview")); }
...
public:
    XRCPreviewEvtHandler(wxWindow* win) : m_window(win) {}
...
protected:
    ...
    void RemoveEventHandler(wxWindow* window)
    {
        ...
        for (size_t i = 0; i < children.GetCount(); ++i) { RemoveEventHandler(children.Item(i)->GetData()); }
...
void XRCPreview::Show(PObjectBase form, const wxString& projectPath)
{
    ...
    AnnoyingDialog dlg(
      _("WARNING - For XRC Developers ONLY!!"),
      wxGetTranslation(
        wxT("The XRC language is not as powerful as C++.\n") wxT("It has limitations that will affect the GUI\n")
      ...
      wxART_WARNING, AnnoyingDialog::OK_CANCEL, wxID_CANCEL);
    ...
    res->SetFlags(res->GetFlags() | wxXRC_NO_SUBCLASSING);
    ...
    if (className == wxT("Frame")) {
        ...
        wxDialog* dialog = new wxDialog(
          wxTheApp->GetTopWindow(), wxID_ANY, wxT("Dialog"), wxDefaultPosition, wxDefaultSize,
...
{
    const wxWindowList& children = window->GetChildren();
    for (size_t i = 0; i < children.GetCount(); ++i) { AddEventHandler(children.Item(i)->GetData(), form); }
