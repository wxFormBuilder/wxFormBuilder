  ...
  m_stopModifiedEvent(false)
{
    ...
    m_back->GetEventHandler()->Connect(
      wxID_ANY, wxEVT_LEFT_DOWN, wxMouseEventHandler(VisualEditor::OnClickBackPanel), NULL, this);
...
void VisualEditor::ScanPanes(wxWindow* parent)
{
    ...
    for (wxWindowList::const_reverse_iterator child = children.rbegin(); child != children.rend(); ++child) {
        ...
        if (obj) {
            ...
            if (
              cname == wxT("widget") || cname == wxT("expanded_widget") || cname == wxT("ribbonbar") ||
              cname == wxT("propgrid") || cname == wxT("propgridman") || cname == wxT("dataviewctrl") ||
              cname == wxT("dataviewtreectrl") || cname == wxT("dataviewlistctrl") || cname == wxT("toolbar") ||
              cname == wxT("container")) {
...
void VisualEditor::ClearWizard()
{
    if (m_wizard) {
        m_wizard->Disconnect(
          wxID_ANY, wxFB_EVT_WIZARD_PAGE_CHANGED, WizardEventHandler(VisualEditor::OnWizardPageChanged));
        m_wizard->Destroy();
        m_wizard = NULL;
    }
}
...
{
    // Individual wxWindow's of composite components made of wxWindow's will be found here as well and won't have an
    // associated ObjectBase, prevent the error log messages
    wxLogNull stopTheLogging;
    const wxWindowList& children = parent->GetChildren();
    for (wxWindowList::const_reverse_iterator child = children.rbegin(); child != children.rend(); ++child) {
        ...
        if (m_form) {
            ...
            if (background && !background->GetValue().empty()) {
                ...
                if (m_form->GetClassName() == wxT("Frame")) {
                    m_back->GetFrameContentPanel()->SetOwnBackgroundColour(
            ...
            // --- [3] Title bar Setup
            if (
              m_form->GetClassName() == wxT("Frame") || m_form->GetClassName() == wxT("Dialog") ||
            ...
            for (unsigned int i = 0; i < m_form->GetChildCount(); i++) {
                ...
                if (!menubar && (m_form->GetObjectTypeName() == wxT("menubar_form"))) {
                    ...
                    menubar = child;
                } else if (toolbar == nullptr && m_form->GetObjectTypeName() == wxT("toolbar_form")) {
...
void VisualEditor::Generate(PObjectBase obj, wxWindow* wxparent, wxObject* parentObject)
{
    ...
    switch (comp->GetComponentType()) {
        case COMPONENT_TYPE_WINDOW:
            ...
            if (NULL == createdWindow) {
                THROW_WXFBEX(wxString::Format(
                  wxT("Component for %s was registered as a window component, but this is not a wxWindow!"),
                  obj->GetClassName()));
            }
            SetupWindow(obj, createdWindow);

            // The event handler must be pushed after OnCreated() because that might push its own event handlers, so
            // record it here only Because wxCollapsiblePane replaces createdWindow the target for the event handler
        ...
        case COMPONENT_TYPE_SIZER: {
            ...
            if (NULL == createdSizer) {
                THROW_WXFBEX(wxString::Format(
                  wxT("Component for %s was registered as a sizer component, but this is not a wxSizer!"),
                  obj->GetClassName()));
            }
    ...
    for (unsigned int i = 0; i < obj->GetChildCount(); i++) { Generate(obj->GetChild(i), new_wxparent, createdObject); }
...
void VisualEditor::SetupWindow(PObjectBase obj, wxWindow* window)
{
    ...
    */
    ...
    if (
      m_auimgr && (tname == wxT("widget") || tname == wxT("expanded_widget") || tname == wxT("container") ||
                   tname == wxT("notebook") || tname == wxT("auinotebook") || tname == wxT("flatnotebook") ||
                   tname == wxT("listbook") || tname == wxT("simplebook") || tname == wxT("choicebook") ||
                   tname == wxT("treelistctrl") || tname == wxT("ribbonbar") || tname == wxT("dataviewctrl") ||
                   tname == wxT("dataviewtreectrl") || tname == wxT("dataviewlistctrl") || tname == wxT("propgrid") ||
                   tname == wxT("propgridman") || tname == wxT("splitter"))) {
...
void VisualEditor::SetupAui(PObjectBase obj, wxWindow* window)
{
    ...
    if (!obj->GetProperty(wxT("aui_name")))
        return;
        ...
        if (obj->GetPropertyAsString(wxT("dock")) == wxT("Dock")) {
            ...
            if (!obj->IsNull(wxT("docking"))) {
                if (obj->GetPropertyAsString(wxT("docking")) == wxT("Bottom"))
                    ...
                    info.Top();
                else if (obj->GetPropertyAsString(wxT("docking")) == wxT("Center"))
                    info.Center();
                else if (obj->GetPropertyAsString(wxT("docking")) == wxT("Right"))
                    info.Right();
            }
        } else {
            info.Float();
    ...
    if (!obj->GetProperty(wxT("pane_size"))->IsNull())
        info.FloatingSize(obj->GetPropertyAsSize(wxT("pane_size")));
    if (!obj->GetProperty(wxT("best_size"))->IsNull())
        info.BestSize(obj->GetPropertyAsSize(wxT("best_size")));
    if (!obj->GetProperty(wxT("min_size"))->IsNull())
        info.MinSize(obj->GetPropertyAsSize(wxT("min_size")));
    if (!obj->GetProperty(wxT("max_size"))->IsNull())
        info.MaxSize(obj->GetPropertyAsSize(wxT("max_size")));
...
void VisualEditor::OnObjectSelected(wxFBObjectEvent& event)
{
    ...
    if (!toolbar)
        toolbar = obj->FindNearAncestor(wxT("toolbar_form"));
    if (toolbar)
        obj = toolbar;
...
{
    bool isSizer =
      (object->GetObjectInfo()->IsSubclassOf(wxT("sizer")) || object->GetObjectInfo()->IsSubclassOf(wxT("gbsizer")));
    int min = (isSizer ? 0 : 1);
    ...
    dc.DrawRectangle(
      point.x - leftBorder, point.y - topBorder, size.x + leftBorder + rightBorder, size.y + topBorder + bottomBorder);
}
...
void DesignerWindow::HighlightSelection(wxDC& dc)
{
    ...
    if (m_selSizer) {
        ...
        if (!sizerParent)
            sizerParent = object->FindNearAncestorByBaseClass(wxT("gbsizer"));
    ...
    for (unsigned int j = 0; j < menu->GetChildCount(); j++) {
        ...
        if (menuItem->GetObjectTypeName() == wxT("submenu")) {
            wxMenuItem* item = new wxMenuItem(
              menuWidget, lastMenuId++, menuItem->GetPropertyAsString(wxT("label")),
              menuItem->GetPropertyAsString(wxT("help")), wxITEM_NORMAL, GetMenuFromObject(menuItem));
