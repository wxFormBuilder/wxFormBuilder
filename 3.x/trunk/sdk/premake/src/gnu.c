...
static int writeRootMakefile()
{
	...
	io_print("  CONFIG=%s\n", gnu_escapePath(prj_get_cfgname()));
	io_print("endif\n\n");
	...
	{
		prj_select_package(i);
		io_print(" %s", gnu_escapePath(prj_get_pkgname()));
	}
	io_print("\n\n");
	...
	for (i = 0; i < prj_get_numpackages(); ++i)
	{
		...
		io_print("\t@$(MAKE) ");
		io_print("--no-print-directory -C %s", gnu_escapePath(path_build(prj_get_path(), prj_get_pkgpath())));
		if (!gnu_pkgOwnsPath())
			io_print(" -f %s.%s", gnu_escapePath(prj_get_pkgname()), DOT_MAKE);
		io_print("\n\n");
	}
