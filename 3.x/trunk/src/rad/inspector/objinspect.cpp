...
<<<<<<< .mine
	AutoGenerateProperty(AppData()->GetSelectedObject(), GetProperty(event.GetFBProperty()), event.GetEventType(), event.GetId());
=======
>>>>>>> .r1589
	m_pg->Refresh();
}
...
{
	AutoGenerateProperty(AppData()->GetSelectedObject(), event.GetPropertyPtr(), event.GetEventType(), event.GetId());
...
void ObjectInspector::AutoGenerateProperty(PObjectBase object, wxPGProperty *prop, wxEventType reason, int eventID)
{
	if(prop)
	{
		if(eventID == WXFB_PROPERTY_GRID)
		{
			if((prop->GetName() == wxT("name") && reason == wxEVT_PG_CHANGED) ||
					(prop->GetName() == wxT("id") && reason == wxEVT_PG_DOUBLE_CLICK))
			{
				AutoGenerateId(object, reason);
			}
			m_pg->Update();
		}
		if(eventID == WXFB_EVENT_GRID)
		{
			AutoGenerateEventHandler(object, prop);
			m_eg->Update();
		}
	}
}

void ObjectInspector::AutoGenerateEventHandler(PObjectBase object, wxPGProperty *evtProp)
{
	if(object)
	{
		wxString eventHandlerName = GenerateEventHandlerName(object, evtProp->GetName());
		evtProp->SetValueFromString(eventHandlerName);
		ObjInspectorEventMap::iterator it = m_eventMap.find(evtProp);
		if ( it != m_eventMap.end() )
		{
			PEvent evt = it->second;
			AppData()->ModifyEventHandler( evt, eventHandlerName );
		}
	}
}

void ObjectInspector::AutoGenerateId(PObjectBase objectChanged, wxEventType reason)
{
	if(objectChanged)
	{
		//wxPGId pgid = m_pg->GetPropertyByLabel(wxT(""));
		PProperty prop = AppData()->GetProjectData()->GetProperty( wxT("event_generation") );
		if ( prop )
		{
			if(prop->GetValueAsString() == wxT("table"))
			{
				...
				if ( prop )
				{
					if(prop->GetValueAsString() == wxT("wxID_ANY") || reason == wxEVT_PG_DOUBLE_CLICK)
					{
						wxString idString = GenerateIDName(objectChanged);
...
wxString ObjectInspector::GenerateIDName(PObjectBase object)
{
	wxString idString;
	PProperty name(object->GetProperty(wxT("name")));
	idString << wxT("ID_");
...
wxString ObjectInspector::GenerateEventHandlerName(PObjectBase object, wxString eventName)
{
	wxString evtString;
	if(object)
	{
		...
		if(name)
		{
			evtString << name->GetValueAsString().Upper();
			evtString << eventName;
		}
	}
	return evtString;
