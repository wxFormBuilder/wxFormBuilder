...
ObjectToXrcFilter::ObjectToXrcFilter(
  IObject* obj, const wxString& classname, const wxString& objname, const wxString& base)
{
    m_obj = obj;
...
void ObjectToXrcFilter::AddProperty(const wxString& objPropName, const wxString& xrcPropName, const int& propType)
{
    ...
    switch (propType) {
        ...
        case XRC_TYPE_BITMAP: {
            ...
            if (
              bitmapProp.StartsWith(_("Load From File")) || bitmapProp.StartsWith(_("Load From Embedded File")) ||
              bitmapProp.StartsWith(_("Load From XRC"))) {
                ...
                propElement.SetAttribute(
                  "stock_id", filename.BeforeFirst(wxT(';')).Trim().Trim(false).mb_str(wxConvUTF8));
                propElement.SetAttribute(
                  "stock_client", filename.AfterFirst(wxT(';')).Trim().Trim(false).mb_str(wxConvUTF8));
  ...
  const wxString& objPropName1, const wxString& objPropName2, const wxString& xrcPropName)
{
    AddPropertyValue(
      xrcPropName, wxString::Format(
                     _("%d,%d"), m_obj->GetPropertyAsInteger(objPropName1), m_obj->GetPropertyAsInteger(objPropName2)));
}
...
XrcToXfbFilter::XrcToXfbFilter(ticpp::Element* obj, const wxString& /*classname*/, const wxString& objname)
{
    m_xrcObj = obj;
...
void XrcToXfbFilter::AddProperty(const wxString& xrcPropName, const wxString& xfbPropName, const int& propType)
{
...
void XrcToXfbFilter::AddPropertyPair(
  const char* xrcPropName, const wxString& xfbPropName1, const wxString& xfbPropName2)
{
    try {
...
void XrcToXfbFilter::ImportFontProperty(const wxString& xrcPropName, ticpp::Element* property)
{
    try {
        ...
        wxString font_str = wxString::Format(
          wxT("%s,%d,%d,%d,%d,%d"), font.GetFaceName().c_str(), font.GetStyle(), font.GetWeight(), font.GetPointSize(),
